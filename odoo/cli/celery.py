...
def extract_settings(options, settings):
    ...
    def get_env(key, override, default=None):
        return override or os.environ.get(key) or default
    ...
    if options.auth:
        settings["oauth"] = {
            "key": get_env("FLOWER_OAUTH2_KEY", options.oauth2_key),
            "secret": get_env("FLOWER_OAUTH2_SECRET", options.oauth2_secret),
