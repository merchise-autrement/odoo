    ...
    def decorator(f):
        ...
        def response_wrap(*args, **kw):
            ...
            if not spec.keywords:
                ignored = [
                    "<%s=%s>" % (k, kw.pop(k)) for k in list(kw) if k not in spec.args
                ]
                if ignored:
                    _logger.info(
...
class Root(object):
    ...
    def load_addons(self):
        ...
        app = IgnoreWeaknessEtagsMiddleware(
            SharedDataMiddleware(self.dispatch, statics, cache_timeout=STATIC_CACHE)
        )
        ...
        if not explicit_session and hasattr(response, "set_cookie"):
            ...
            response.set_cookie(
                ...
                domain=self._get_matching_domain(
                    httprequest, get_conf("session_cookie_domain", "")
                ),
