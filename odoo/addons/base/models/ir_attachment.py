...
class IrAttachment(models.Model):
    ...
    def _check_contents(self, values):
        ...
        force_text = xml_like and (
            not user._is_system() or self.env.context.get("attachments_mime_plainxml")
        )
        if force_text:
    ...
    cache_control_header = fields.Char(
        "Cache Control Header (as is)", size=200, index=False, readonly=True
    )
