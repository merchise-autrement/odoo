...
class product_pricelist(osv.osv):
    ...
    def _get_rule(self, cr, uid, pricelist_id, product, qty_in_product_uom, context=None):
        context = context or {}
        ...
        cr.execute(
            ...
            (prod_tmpl_id, prod_ids, categ_ids, pricelist_id, date, date))
        row = cr.fetchone()
        if row:
    ...
    def get_base_price(self, cr, uid, product, rule, qty,
                       pricelist, partner=None, context=None):
        price = False
        if rule.base == 'pricelist' and rule.base_pricelist_id:
            price_tmp = self._price_get_multi(
                cr, uid, rule.base_pricelist_id, [(product, qty, partner)],
            ...
            # if base option is public price take sale price else cost price
            # of product price_get returns the price in the context UoM,
            # i.e. qty_uom_id
            price = self.pool['product.template']._price_get(
                cr, uid, [product], rule.base, context=context)[product.id]
        return price

    def get_final_price(self, cr, uid, product, rule, price, price_uom_id):
    ...
    def _price_rule_get_multi(self, cr, uid, pricelist,
        ...
        for product, qty, partner in products_by_qty_by_partner:
            ...
            # contains that UoM.  The final price will be converted to match
            # `qty_uom_id`.
            ...
            rule = self._get_rule(
                cr, uid, pricelist.id, product, qty_in_product_uom, context=context
            )
            if rule:
                price = self.get_base_price(
                    cr, uid, product, rule, qty, pricelist, partner=partner, context=context
                )
                if price is not False:
                    price = self.get_final_price(cr, uid, product, rule, price, price_uom_id)
                    suitable_rule = rule
