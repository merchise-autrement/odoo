...
def _get_valid_version(versions, date):
    for v in versions:
        if (v.date_start is False) or (v.date_start <= date):
            if(v.date_end is False) or (v.date_end >= date):
                return v
    ...
    def get_base_price(self, cr, uid, product, rule, qty,
        ...
        if rule.base == -1:
            ...
            if rule.base not in price_types:
                price_types[rule.base] = price_type_obj.browse(cr,
                                                               uid,
                                                               int(rule.base))
