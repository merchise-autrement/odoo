...
def WAIT_FOR_TASK(
    ...
    next_action=None,
    cancellable=False,
    stages: Sequence[IntoProgressStage] = None,
):
...
class Module(models.Model):
    ...
    def button_immediate_install(self):
        if (
            ...
            or not config.get("dev_mode")
            or not odoo.multi_process
        ):
            report_progress(
                message=_(
                    "Installation has begun, wait for a minute or two to finish."
                ),
                progress=0,
                valuemin=0,
                valuemax=100,
...
class CeleryController(http.Controller):
    @http.route(
        "/web_celery/!cancel/<uuid:identifier>",
        auth="user",
        type="json",
        methods=["POST"],
    )
