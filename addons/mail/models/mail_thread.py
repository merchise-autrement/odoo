...
class MailThread(models.AbstractModel):
    ...
    def _get_followers(self):
        for thread in self:
            ...
            thread.message_channel_ids = thread.message_follower_ids.mapped(
                "channel_id"
            )
    ...
    def _search_message_has_error(self, operator, operand):
        return [
            "&",
            ("message_ids.has_error", operator, operand),
    ...
    def create(self, vals_list):
        ...
        if not self._context.get("mail_create_nosubscribe"):
            for values in vals_list:
                ...
                message_follower_ids += [
                    (0, 0, fol_vals)
                    for fol_vals in self.env["mail.followers"]._add_default_followers(
        ...
        if not self._context.get("mail_create_nolog"):
            ...
            for thread in threads:
                ...
                if (
                    subtype
                ):  # if we have a sybtype, post message to notify users from _message_auto_subscribe
        ...
        if not self._context.get("mail_notrack"):
            ...
            for thread in track_threads:
                ...
                changes = [
                    field for field in tracked_fields if create_values.get(field)
                ]
    ...
    def write(self, values):
        ...
        if tracked_fields:
            initial_values = dict(
                (record.id, dict((key, getattr(record, key)) for key in tracked_fields))
    ...
    def get_empty_list_help(self, help):
            ...
            if "oe_view_nocontent_alias" not in help:
                return (
                    ...
                    % {
                        "static_help": help,
                        "dyn_help": _(
                            "Create a new %(document)s by sending an email to %(email_link)s"
                        )
                        % {"document": document_name, "email_link": email_link},
                    }
                )
        ...
        if nothing_here:
            return "<p class='o_view_nocontent_smiling_face'>%(dyn_help)s</p>" % {
                "dyn_help": _("Create a new %(document)s") % {"document": document_name}
            }
    ...
    def message_track(self, tracked_fields, initial_values):
        ...
        tracking = dict()
        for record in self:
            tracking[record.id] = record._message_track(
   ...
   Only allowed people can contact us at this address.</p>
        ...
        if thread_id:
            if not record_set.exists():
                self._routing_warn(
                    _(
                        "reply to missing document (%s,%s), fall back on document creation"
                    )
                    % (model, thread_id),
                    message_id,
                    route,
        ...
        if alias:
            ...
            if hasattr(obj, "_alias_check_contact"):
                ...
                check_result = self.env[
                    "mail.alias.mixin"
                ]._alias_check_contact_on_record(obj, message, message_dict, alias)
            if check_result is not True:
                self._routing_warn(
                    _("alias %s: %s")
                    % (
                        alias.alias_name,
                        check_result.get("error_message", _("unknown error")),
                    ),
                    message_id,
                    route,
                    False,
                ...
                self._routing_create_bounce_email(
                    email_from,
                    check_result.get("error_template", _generic_bounce_body_html),
                    message,
                )
                return False
    ...
    def message_route(
        ...
        mail_messages = (
            ...
            .sudo()
            .search([("message_id", "in", msg_references)], limit=1)
        )
        ...
        if rcpt_tos_localparts:
            ...
            if catchall_alias and catchall_alias in email_to_localpart:
                ...
                self._routing_create_bounce_email(
                    email_from, body, message, reply_to=self.env.company.email
                )
                return []
    ...
    def message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            MessageModel = Model.with_user(user_id).with_context(
        ...
        if message.get_content_maintype() == "text":
            ...
            for part in message.walk():
                ...
                # 4-bytes attachments with "None".
                    ...
                    body = tools.append_content_to_html(
                        body,
                        tools.ustr(payload, encoding, errors="replace"),
                        preserve=True,
                    )
    ...
    def _message_compute_author(
        ...
        return {"author_id": author_id, "email_from": email_from}
    ...
    def _message_create(self, values_list):
        ...
        for values in values_list:
            ...
            create_values["channel_ids"] = [
                (4, cid) for cid in create_values.get("channel_ids", [])
            ]
        ...
        if inbox_pids or channel_ids:
            ...
            if channel_ids:
                bus_notifications += (
                    ...
                    .sudo()
                    .browse(channel_ids)
                    ._channel_message_notifications(message, message_format_values)
                )
    ...
    def _notify_prepare_template_context(
        ...
        company = (
            self.company_id.sudo() if self and "company_id" in self else user.company_id
        )
        if company.website:
            website_url = (
        ...
        is_discussion = subtype_id == self.env["ir.model.data"].xmlid_to_res_id(
            "mail.mt_comment"
        )
    ...
    def _notify_compute_recipients(self, message, msg_vals):
        ...
        recipient_data = {"partners": [], "channels": []}
    ...
    @api.model
    def _notify_get_reply_to_on_records(
        self, default=None, records=None, company=None, doc_names=None
    ):
        ...
        if records and hasattr(records, "_notify_get_reply_to"):
            return records._notify_get_reply_to(
                default=default, company=company, doc_names=doc_names
            )
        return self._notify_get_reply_to(
            default=default, records=records, company=company, doc_names=doc_names
        )
    ...
    def _notify_email_recipient_values(self, recipient_ids):
        ...
        """
        return {"email_to": False, "recipient_ids": recipient_ids}
    ...
    def message_subscribe(self, partner_ids=None, channel_ids=None, subtype_ids=None):
        ...
        return self._message_subscribe(
            partner_ids, channel_ids, subtype_ids, customer_ids=customer_ids
        )
    ...
    def _message_subscribe(
        ...
        if not subtype_ids:
            ...
            self.env["mail.followers"]._insert_followers(
                ...
                channel_ids,
                dict((cid, subtype_ids) for cid in channel_ids),
                customer_ids=customer_ids,
                check_existing=True,
    ...
    def _message_auto_subscribe_notify(self, partner_ids, template):
        ...
        for record in self:
            ...
            values = {"object": record, "model_description": model_description}
    ...
    def _message_auto_subscribe(self, updated_values):
        ...
        all_ids, def_ids, int_ids, parent, relation = self.env[
        ...
        if udpated_fields:
            ...
            for fid, rid, pid, cid, subtype_ids, pshare in res:
                ...
                sids += [
                    sid for sid in subtype_ids if sid not in parent and sid in def_ids
                ]
                if pid:
                    new_partners[pid] = (
                        (set(sids) & set(all_ids)) - set(int_ids)
                        if pshare
                        else set(sids) & set(all_ids)
                    )
                if cid:
