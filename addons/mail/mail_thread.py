...
class mail_thread(osv.AbstractModel):
    ...
    def create(self, cr, uid, values, context=None):
        ...
        if context.get('tracking_disable'):
            ...
            # thread index.
            if thread_id:
                self._ensure_index(cr, uid, thread_id, context=context)
            return thread_id
        ...
        # merchise: Ensure to have the unique index before trying to create
        # notifications, so that those notification have the proper thread
        # index.
        if thread_id:
            self._ensure_index(cr, uid, thread_id, context=context)
