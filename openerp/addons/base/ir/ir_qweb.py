...
class QWeb(orm.AbstractModel):
    ...
    * alternatively, override :meth:`~.get_converter_for` and return an
    ...
    def render_tag_call_assets(self, element, template_attributes, generated_attributes, qwebcontext):
        ...
        return bundle.to_html(css=css, js=js, debug=bool(qwebcontext.get('debug')), async=async, qwebcontext=qwebcontext, spdy=spdy)
...
class AssetsBundle(object):
    ...
    def to_html(self, sep=None, css=True, js=True, debug=False, async=False, qwebcontext=None, spdy=False):
        if sep is None:
            sep = '\n            '
        response = []
        if debug or spdy:
        ...
        for atype in (SassStylesheetAsset, LessStylesheetAsset):
            ...
            if assets:
                ...
                while fragments:
                    ...
                    if debug or spdy:
                        # TODO: Check if this is truly needed for SPDY.
                        try:
                            ira = self.registry['ir.attachment']
